...
public class Printer
{
    ...
    public static ArrayList<String> usecase = new ArrayList<String>();
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        {
            source += "skinparam usecaseBackgroundColor #A80036\n" + "skinparam usecaseBorderColor Transparent\n"
                    + "skinparam usecaseFontSize 1\n" + "skinparam usecaseFontColor #A80036\n";
            for (String i : interfaces)
            {
                usecase.add(i);
                String case1 = "case" + i + "1";
                String case2 = "case" + i + "2";
                usecase.add(case1);
                usecase.add(case2);
            }
        }
        ...
        for (int i = 0; i < listOfFiles.length; i++)
        {
            ...
            if (file.isFile() && file.getName().endsWith(".java"))
            {
                ...
                if (uses.size() > 1 && classes.contains(uses.get(0)))
                {
                    ...
                    {
                        String dependency = uses.get(0) + RelationSymbol.dependency.toString() + uses.get(j) + "\n";
        ...
        //dealing with ball & socket
        if (dependent.size() > 0)
        {
            for (String i : interfaces)
            {
                boolean isused = false;
                boolean isimple = false;
                for (String d : dependent)
                {
                    if (d.contains("case" + i + "2"))
                    {
                        isused = true;
                    }
                }
                for (String im : implementation)
                {
                    if (im.contains("case" + i + "1"))
                    {
                        isimple = true;
                    }
                ...
                if (isused)
                {
                    int index = usecase.indexOf(i);
                    source += "mix_usecase " + usecase.get(index + 1) +" \n";
                    source += "mix_usecase " + usecase.get(index + 2) +" \n";
                    source += usecase.get(index + 1) + RelationSymbol.ballnsocket.toString() +
                            usecase.get(index + 2) + " : " + i + "\n";
                    //System.out.println(source);
                }
                else if (isimple)
                {
                    int index = implementation.indexOf("case" + i + "1");
                    implementation.set(index - 1, "-()");
                    implementation.set(index, i);
                }
            }
        }
        ...
        if(relation.toString().contains("-"))
        {
            ...
            for(String print : removeduplicate)
            {
                ...
                String first = print.substring(0,print.indexOf("\""));
                String last = print.substring(print.lastIndexOf("\"")+1, print.length());
        ...
        //System.out.println(source);
        source += "\n" + "title Huimin Jian 010129561 ball&socket\n" + "@enduml\n";
    ...
    private static class ClassVisitor extends VoidVisitorAdapter
    {
        ...
        public void visit(ClassOrInterfaceDeclaration n, Object arg)
        {
            if (n.isInterface())
            {
                ...
                {
                    uses.add(usecase.get(usecase.indexOf(n.getName()) + 2));
                    ...
                    {
                        relation.append(usecase.get(usecase.indexOf(n.getName()) + 1));
                    }
                    else
                    {
                        relation.append("\n" + usecase.get(usecase.indexOf(n.getName()) + 1));
                    }
                }
                else
                {
                ...
                if (classes.contains(n.getName()))
                {
                    ...
                    if(n.getImplements() != null)
                    {
                        ...
                        for(int i = 0 ; i < impleArray.length;i++)
                        {
                            ...
                            implementation.add(usecase.get(usecase.indexOf(impleArray[i]) + 1));
                            implementation.add("\n");
                        }
                    }
        ...
        public void visit(MethodDeclaration n, Object arg)
        {
            ...
            else
            {
                ...
                if (space > 0)
                {
                    ...
                    {
                        uses.add(usecase.get(usecase.indexOf(useinterface) + 2));
                    }
    ...
    private static class FieldVisitor extends VoidVisitorAdapter
    {
        ...
        public void visit(FieldDeclaration n, Object arg)
        {
            ...
            else
            {
                if (n.getType().toString().contains("<"))     //find "< >" and replace with "*"
                {
                    ...
                    else if (interfaces.contains(symbol))
                    {
                        relation.append("\" \"" + RelationSymbol.association.toString()+"\"*\""
                                + usecase.get(usecase.indexOf(symbol) + 1) + "\n");
                ...
                else
                {
                    ...
                    else if (interfaces.contains(n.getType().toString()))
                    {
                        relation.append("\" \"" + RelationSymbol.association.toString() + "\" \""
                                + usecase.get(usecase.indexOf(n.getType().toString()) + 1) + "\n");
                    }
                    else
                    {
        ...
        public void visit(VariableDeclarationExpr n, Object arg)
        {
            ...
            {
                uses.add(usecase.get(usecase.indexOf(n.getType().toString()) + 2));
            }
        }
    }
